...
function createWindow() {
  ...
  const iconIcoPath = path.join(iconBasePath, "app-icon.ico");
  const iconPngPath = path.join(iconBasePath, "app-icon.png");
  ...
  if (isDev) {
    ...
    mainWindow.loadFile(indexPath).catch((error) => {
      console.error("Failed to load renderer file:", indexPath, error);
    });
  }

  mainWindow.webContents.on("did-fail-load", (_, code, desc, url) => {
    console.error(`Renderer failed to load (code=${code}) ${desc} :: ${url}`);
  });
...
async function registerIpc() {
    ...
    if (payload.fromDate || payload.toDate) {
      and.push({
        dueDate: {
          ...
          lte: payload.toDate ? toDateEndOfDay(payload.toDate) : undefined
        }
      });
    }
  ...
  ipcMain.handle("history:list", async (_, payload = {}) => {
    const walletId = payload.walletId || undefined;
    const search = String(payload.search || "").trim();
    const tagId = String(payload.tagId || "").trim();
    const type = payload.type || undefined;
    const fromDate = payload.fromDate ? toDate(payload.fromDate) : undefined;
    const endDate = payload.toDate ? toDateEndOfDay(payload.toDate) : undefined;
    const limit = Math.max(50, Math.min(Number(payload.limit || 800), 3000));
    ...
    const txWhere = {
      ...
      dueDate: fromDate || endDate ? { gte: fromDate, lte: endDate } : undefined
    };
    ...
    const [txRows, occurrenceRows] = await Promise.all([
      prisma.transaction.findMany({
        where: txWhere,
    ...
    const txItems = txRows.map((item) => ({
      id: `tx:${item.id}`,
      source: "TRANSACTION",
      sourceLabel: item.sourcePlanId ? "Transação (de plano)" : "Transação",
      type: item.type,
      description: item.description || "Sem descrição",
      amount: item.amountCents / 100,
      dueDate: item.dueDate,
      paidAt: item.paidAt,
      tags: (item.tags || []).map((link) => link.tag).filter(Boolean)
    }));

    const occurrenceItems = occurrenceRows.map((item) => ({
      id: `occ:${item.id}`,
      source: "PLAN_OCCURRENCE",
      sourceLabel: item.plan?.type === "RECURRING" ? "Fixo / Plano" : "Plano parcelado",
      type: item.plan?.transactionType || "EXPENSE",
      description: item.plan?.description || "Sem descrição",
      amount: item.amountCents / 100,
      dueDate: item.dueDate,
      paidAt: null,
      tags: (item.plan?.tags || []).map((link) => link.tag).filter(Boolean)
    }));

    return [...txItems, ...occurrenceItems]
  ...
  ipcMain.handle("goal:allocate", async (_, payload) =>
    prisma.$transaction(async (tx) => {
      ...
      if (!amountCents) throw new Error("Valor do aporte inválido.");
      ...
      let metasTag = await tx.tag.findFirst({
        where: { walletId: goal.walletId, name: "Metas" },
        select: { id: true, isArchived: true }
      });
      if (!metasTag) {
        metasTag = await tx.tag.create({
          data: { walletId: goal.walletId, name: "Metas", color: "#f59e0b" },
          select: { id: true, isArchived: true }
        });
      } else if (metasTag.isArchived) {
        metasTag = await tx.tag.update({
          where: { id: metasTag.id },
      ...
      const now = new Date();
      const isWithdraw = amountCents < 0;
      const movement = Math.abs(amountCents);
      const transaction = await tx.transaction.create({
        data: {
          walletId: goal.walletId,
          type: isWithdraw ? "INCOME" : "EXPENSE",
          description: isWithdraw ? `Retirada da meta: ${goal.name}` : `Aporte na meta: ${goal.name}`,
      ...
      await tx.transactionTag.create({
        data: { transactionId: transaction.id, tagId: metasTag.id }
      });

      return tx.goalAllocation.create({
        data: {
